### GSE157827 Numba-capped full-pipeline repeatability — STILL UNSTABLE
- Run 36174283860 completed two independent replicas with BLAS/OpenMP thread pools and NUMBA_NUM_THREADS all capped at one.
- Replicate 1 produced 29 clusters; replicate 2 produced 31 clusters.
- The two replicas did not produce byte-identical Harmony20 files, so capping Numba did not make the full upstream pipeline deterministic.
- This does not overturn the earlier stronger localization from run 36173634884, where two replicas had byte-identical Harmony20 but different 29-vs-31 partitions. It shows that both upstream floating-point/runtime drift and downstream approximate-neighbor/partition drift can occur across hosted runners.
- Next decisive test: hold one archived Harmony20 matrix byte-for-byte fixed across two independent jobs, cap all numerical and Numba threads, and rerun only neighbors plus Leiden. This directly tests downstream determinism without rebuilding HVG/PCA/Harmony.
...
Durable checkpoint so the RETINA-ND manuscript audit/extension can be resumed in another analysis session if Work mode credits run out.
