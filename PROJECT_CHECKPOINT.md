### Publication source-data preservation — independent PXD040225 AD-retina block COMPLETE
- Run 36192274476 completed successfully and committed exact durable PXD040225 publication source data at commit 0591e45.
- Preserved the extracted significant retinal DEP table, the prespecified eight-candidate overlap table, and extraction summary/provenance.
- Verification: 845 extracted DEP rows, 844 unique genes; 423 Table-4 up rows and 422 Table-5 down rows.
- Of the eight prespecified manuscript candidates, only ITGAM overlaps: P11215, AD mean 79.11, control mean 62.72, AD/control fold change 1.26, P=0.03222, signed log2FC +0.333424.
- Important limitation preserved explicitly: the paper reports 886 retinal DEPs overall, so the 844-unique-gene extraction from Tables 4/5 is not represented as a complete reconstruction of every published retinal DEP.
- Interpretation remains nominal independent human AD-retina support for ITGAM, not FDR biomarker evidence.
- Refreshed publication/source_data/SHA256SUMS.txt. Main and Zenodo remain unchanged.
- Next source-data block should preserve the original publication-facing pathway and multimodal-candidate source tables before any workbook or figure regeneration.

### Publication source-data preservation — vascular block COMPLETE
- Run 36192137147 completed successfully and committed exact durable vascular source data at commit e626c23.
- Preserved 9 vascular comparison summaries and all 9 merged gene-level comparison tables, plus donor/effect/pseudobulk tables generated by the vascular workflow.
- Preserved all 4 endothelial donor-bootstrap summaries; every 95% interval crosses zero as frozen.
- Preserved the GSE157827 author-Endo effect vector and all 4 merged sensitivity comparisons.
- Automated verification matched the frozen values for AD endothelial/pericyte replication, PD endothelial non-replication, all cross-disease vascular comparisons, endothelial bootstrap intervals, and author-Endo sensitivity results.
- Added subdirectory provenance READMEs and refreshed the repository-wide publication/source_data/SHA256SUMS.txt.
- Source artifacts: vascular run 36142316148 / artifact 10867267704; author-Endo run 36147039005 / artifact 10869811622.
- Main and Zenodo remain unchanged.
- Next single source-data block: preserve and verify the independent human AD-retina PXD040225 sensitivity outputs.
